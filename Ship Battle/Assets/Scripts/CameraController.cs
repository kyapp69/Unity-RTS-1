...
public class CameraController : MonoBehaviour {
	...
	public float xboundary = 4000f;
	public float zboundary = 4000f;
	...
	void MoveCamera() {
		...
		if (!Input.GetKey(KeyCode.LeftControl)) {
			// mouse left
			if ((mouseX < scrollDistance || Input.GetKey(KeyCode.A)) && x > -xboundary) {
				transform.Translate(-scrollAmount, 0, 0);
			}
			// mouse right
			if ((mouseX >= Screen.width - scrollDistance || Input.GetKey (KeyCode.D)) && x < xboundary) {
				transform.Translate(scrollAmount, 0,0);
			}
			// mouse down
			if ((mouseY < scrollDistance || Input.GetKey(KeyCode.S)) && z > -zboundary) {
				transform.Translate(0,0, -scrollAmount);
			}
			// mouse up
			if ((mouseY >= Screen.height - scrollDistance || Input.GetKey(KeyCode.W)) && z < zboundary) {
				transform.Translate(0,0,scrollAmount);
			}
		}
	}
